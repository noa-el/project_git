function [r,v,box]=make_fcc_lattice4(L,d,theta)
...
%---periodic boundaries work only for theta=45;

%index=find(r(:,1)>L(1)/2-epsilon | r(:,1)<-L(1)/2-epsilon | r(:,2)>L(2)/2-epsilon | r(:,2)<-L(2)/2-epsilon | r(:,3)>(2*n_z-2)*d-epsilon);
index=find(r(:,1)>L(1)/2-epsilon | r(:,1)<-L(1)/2-epsilon | r(:,2)>L(2)/2-epsilon | r(:,2)<-L(2)/2-epsilon | r(:,3)<epsilon);
r(index,:)=[];
